...
data1 = np.genfromtxt(f'data/{header.method}_{header.func}_h=0.01_c={header.courant}_frpersave={header.frames_per_save}.csv', delimiter=',')
print('read')
x1 = data1[0]
data1 = np.delete(data1, 0, axis=0)
data2 = np.genfromtxt(f'data/{header.method}_{header.func}_h=0.001_c={header.courant}_frpersave={header.frames_per_save}.csv', delimiter=',')
print('read')
x2 = data2[0]
data2 = np.delete(data2, 0, axis=0)[::10] #10 - отношение шагов 0.01 / 0.001
...
def update(frame):
    ...
    ttl2.set_text(f'Погрешность, отношение={_error1 / _error2:.1f}')
...
ani = FuncAnimation(fig=fig, init_func=init, func=update, frames=len(data1), interval=1, repeat=False)
plt.show()
