...
with open(f'data/{header.method}_h=0.01_c={header.courant}_frpersave={header.frames_per_save}.csv', newline='') as f1, \
        open(f'data/{header.method}_h=0.001_c={header.courant}_frpersave={header.frames_per_save}.csv', newline='') as f2:
    ...
    if k:
        plt.show()
        print(f'0.01: {max(error1)}')
        print(f'0.001: {max(error2)}')
    else:
